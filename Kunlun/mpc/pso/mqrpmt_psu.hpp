...
void Send(NetIO &io, PP &pp, std::vector<block> &vec_X,
          std::vector<EC25519Point>* out_vec_Fk2_X = nullptr)
{
    ...
    cwPRFmqRPMT::Client(io, pp.mqrpmt_part, vec_X, out_vec_Fk2_X);
...
std::vector<block> Receive(NetIO &io, PP &pp, std::vector<block> &vec_Y,
                     std::vector<EC25519Point>* out_vec_Fk1_Y = nullptr)
{
    ...
    PrintSplitLine('-');
    std::cout << "[mqRPMT-based PSU] Phase 1: execute mqRPMT >>>" << std::endl;
    std::vector<uint8_t> vec_indication_bit = cwPRFmqRPMT::Server(io, pp.mqrpmt_part, vec_Y, out_vec_Fk1_Y);
