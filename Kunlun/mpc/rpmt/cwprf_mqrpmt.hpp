...
std::vector<uint8_t> Server(NetIO &io, PP &pp, std::vector<block> &vec_Y, std::vector<ECPoint>* out_vec_Fk1_Y = nullptr)
{
    ...
    // 可选出参:拷贝密文给调用者(不修改原行为)
    if (out_vec_Fk1_Y != nullptr) {
        *out_vec_Fk1_Y = vec_Fk1_Y;
    }
...
void Client(NetIO &io, PP &pp, std::vector<block> &vec_X, std::vector<ECPoint>* out_vec_Fk2_X = nullptr)
{
    ...
    // 可选出参:拷贝密文给调用者(不修改原行为)
    if (out_vec_Fk2_X != nullptr) {
        *out_vec_Fk2_X = vec_Fk2_X;
    }
...
std::vector<uint8_t> Server(NetIO &io, PP &pp, std::vector<block> &vec_Y, std::vector<EC25519Point>* out_vec_Fk1_Y = nullptr)
{
    if(pp.SERVER_LEN != vec_Y.size()){
        std::cerr << "input size of vec_Y does not match public parameters" << std::endl;
        exit(1);
...
void Client(NetIO &io, PP &pp, std::vector<block> &vec_X, std::vector<EC25519Point>* out_vec_Fk2_X = nullptr)
{
    if(pp.CLIENT_LEN != vec_X.size()){
