...
void SaveSenderResult(const std::string& filename) {
    ...
    fout << "Sender finished successfully." << std::endl;
    fout.close();
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "    PSI-Card Sender" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "[PSI-Card] Running as SENDER..." << std::endl;
    PrintSplitLine('-');
    
    NetIO client("client", "127.0.0.1", 8080);
    std::vector<EC25519Point> vec_Fk2_X;
    mqRPMTPSIcard::Send(client, pp, testcase.vec_X_padded, &vec_Fk2_X);

    // 发送方结果保存到小组目录
    std::string result_file = KUNLUN_BASE_DIR "/PSO_data/PSI_card_data/group_" + g_group_id + "/sender_result_card.txt";
    SaveSenderResult(result_file);

    // 保存己方 OPRF 密文（让前端可以展示"密文面板"）
    std::string ciphertext_file = KUNLUN_BASE_DIR "/PSO_data/PSI_card_data/group_" + g_group_id + "/sender_ciphertext.txt";
    SaveOPRFCiphertext(ciphertext_file, vec_Fk2_X);
    
    std::cout << "[PSI-Card] Sender finished." << std::endl;
    PrintSplitLine('-');
