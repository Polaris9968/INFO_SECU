...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[PSI] Running as SENDER..." << std::endl;
    PrintSplitLine('-');

    NetIO client("client", "127.0.0.1", 8080);

    // 调用 PSI.Send，拿 OPRF 编码后的密文 (vec_Fk2_X)
    std::vector<EC25519Point> vec_Fk2_X;
    mqRPMTPSI::Send(client, pp, testcase.vec_X_padded, &vec_Fk2_X);

    // 保存 OPRF 密文（真正的密文，给前端展示）
    std::string ciphertext_file = KUNLUN_BASE_DIR "/PSO_data/PSI_data/group_" + g_group_id + "/sender_ciphertext.txt";
    SaveOPRFCiphertext(ciphertext_file, vec_Fk2_X);

    // 发送方结果保存到小组目录
    std::string result_file = KUNLUN_BASE_DIR "/PSO_data/PSI_data/group_" + g_group_id + "/sender_result.txt";
    SaveSenderResult(result_file);
    
    std::cout << "[PSI] Sender finished." << std::endl;
    PrintSplitLine('-');
