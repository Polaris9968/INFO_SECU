...
// ============================================================================
// 1. 自定义 Block 比较器 / 字符串映射（与 PSI-Card / PSI-Sum sender 同源）
// ============================================================================
...
// ============================================================================
// 2. 数据结构（receiver 视角，不需要 value 向量）
// ============================================================================
...
// ============================================================================
// 4. 从文件加载数据（receiver 只读 receiver.txt）
// ============================================================================
...
MyTestCase LoadTestCaseFromFiles() {
    ...
    // ---- 读取 sender 集合（receiver 只是看一下，不参与 OPRF） ----
...
// ============================================================================
// 5. 填充到 2 的幂次（receiver 与 sender 用不同的 dummy_seed 防 dummy 命中）
// ============================================================================
...
void SaveCardinalityToFile(size_t cardinality, const std::string& filename) {
...
void SaveReceiverResult(const std::string& filename) {
    std::ofstream fout(filename);
    if (!fout) {
        std::cerr << "Error: Cannot open " << filename << std::endl;
        return;
    }
    fout << "PSI-Sum Receiver finished successfully." << std::endl;
    fout.close();
    std::cout << "[Save] Receiver completed." << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "    PSI-Sum Receiver" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "[PSI-Sum] Running as RECEIVER..." << std::endl;
    PrintSplitLine('-');

    // PSI-Sum 的 receiver 是 client (连过去 8080)
    NetIO client("client", "127.0.0.1", 8080);

    std::vector<EC25519Point> vec_Fk1_Y;
    size_t cardinality = mqRPMTPSIcardsum::Receive(
        client, pp,
        testcase.vec_Y_padded
    );
    ...
    // cardinality（receiver 视角的结果，同时冗余写一份 cardinality.txt 供后端读取）
    SaveCardinalityToFile(cardinality, group_dir + "/receiver_cardinality.txt");
    SaveCardinalityToFile(cardinality, group_dir + "/cardinality.txt");
    SaveReceiverResult(group_dir + "/receiver_result.txt");

    std::cout << "[PSI-Sum] Receiver finished." << std::endl;
    PrintSplitLine('-');
